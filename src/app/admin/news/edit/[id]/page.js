...
export default function EditNewsPost({ params }) {
  ...
  // Get current admin and post data on component mount
  useEffect(() => {
    const init = async () => {
    ...
    try {
      ...
      if (data.user) {
        ...
        if (adminError) {
          ...
          throw new Error('You must be logged in as an admin to edit news posts');
        }
      ...
      setError(err.message || 'You must be logged in as an admin to edit news posts');
      router.push('/admin/login');
    }
  };
  ...
  const fetchPostData = async () => {
    ...
    try {
      ...
      // Set the form data with the post data
      setFormData({
        title: data.title || '',
        slug: data.slug || '',
      ...
      // Set the existing image URL if there is one
      if (data.image_url) {
        setExistingImageUrl(data.image_url);
      }
    } catch (err) {
      console.error('Error fetching post:', err);
      setError('Error loading post data. Please try again.');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!admin) {
      setError('You must be logged in as an admin to edit a post');
      return;
    }
    ...
    try {
      ...
      console.log('Attempting to update post data:', postId, { 
        ...postData, 
        content: postData.content.length + ' chars',
        image_url: imageUrl ? (imageUrl === existingImageUrl ? 'Unchanged image' : 'New base64 image (truncated)') : null
      });
      ...
      if (updateError) {
        ...
        throw new Error(updateError.message || 'Failed to update news post');
      }

      if (!data || data.length === 0) {
        console.error('No data returned after update');
        throw new Error('Failed to update post: No data returned from database');
      }
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-white rounded-lg shadow p-4 sm:p-6">
            <div className="flex items-center justify-center h-64">
              ...
              <span className="ml-3 text-lg text-gray-600">Loading post data...</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          <div className="mb-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Edit News Post</h1>
                <p className="text-sm sm:text-base text-gray-600">Make changes to your news post</p>
              </div>
              <button
                type="button"
              ...
              <div>
                <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                  ...
                  <div className="sm:col-span-4">
                    ...
                    <p className="mt-2 text-sm text-gray-500">URL-friendly version of the title.</p>
                  </div>
                        ...
                        <div className="mr-4">
                          ...
                          <div className="relative">
                            {existingImageUrl.startsWith('data:image') ? (
                              <img src={existingImageUrl} alt="Current" className="h-32 w-auto object-cover rounded-md" />
                            ) : (
                              <img src={existingImageUrl} alt="Current" className="h-32 w-auto object-cover rounded-md" />
                            )}
                          </div>
